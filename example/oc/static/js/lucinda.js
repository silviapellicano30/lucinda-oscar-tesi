class Lucinda_util {
      ...
      static get_hf_callfun(hfconf) {
        let res = [];
        for (let i = 0; i < hfconf.length; i++) {
          if ("callfun" in hfconf[i]){
            res.push( hfconf[i] );
          }
        }
        return res;
      ...
      static get_hf_callsparql(hfconf) {
        let res = [];
        for (let i = 0; i < hfconf.length; i++) {
          if ("sparql" in hfconf[i]){
            res.push( hfconf[i] );
          }
        }
        return res;
  ...
  eval_command(s) {
    ...
    let parsedExpression = s.replace(regex, (match, variable, operator, value) => {
      if (variable && operator && value) {
        ...
        var_value = this.getVal(var_value);
...
class Lucinda {
      ...
      function _do_queries( ) {
        ...
        for (let i = 0; i < l_callsparql.length; i++) {
            ...
            if ( (cr_query_block.id == undefined) || (cr_query_block.endpoint == undefined) || (cr_query_block.sparql == undefined) || (cr_query_block.method == undefined) ) {
              console.log("Error: some mandatory fields (id, endpoint, sparql, method) are not defined in the configuration file.");
              return null;
            }
        ...
        fetch(query_call.call,query_call.args)
          ...
          .then(data => {
            ...
            if (res_normal == null) {
              ...
              //   Lucinda.build_success_html_page();
              // }
              // when all pending sparql queries are done build the HTML page
              // exclude callfunctions
              const excluded = Lucinda_util.get_hf_callfun(Lucinda.current_resource["hfconf"]).map(item => item.id);
              if (
                Object.entries(Lucinda.data)
    ...
    static run_extdata(){
      ...
      const l_callfun = Lucinda_util.get_hf_callfun(Lucinda.current_resource.hfconf);

      for (let i = 0; i < l_callfun.length; i++) {
        const callfun_block = l_callfun[i];
        _call_extfun(callfun_block.id, callfun_block.callfun);
      }
